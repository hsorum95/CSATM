...
def create_ad_tree(resource_name: str,threat_name: str) -> None:
    ...
    if int(threat_id) == 10:
        ...
        attack2 = pydot.Node(f'Abusing over scoped network access', shape='box')
        dot.add_node(attack2)
        dot.add_edge(pydot.Edge(root,attack2))
        attack3 = pydot.Node(f'Inside threat exfiltrates data', shape='box')
        dot.add_node(attack3)
        dot.add_edge(pydot.Edge(root,attack3))
        #2lvl
        attack4 = pydot.Node(f'SSRF on server', shape='box')
        dot.add_node(attack4)
        dot.add_edge(pydot.Edge(attack2,attack4))
        #defenses
        def1 = pydot.Node(f'Data Protection by Design and Default')
        def2 = pydot.Node(f'Least Privilege')
        def3 = pydot.Node(f'User Access Review')
        dot.add_edge(pydot.Edge(attack,def2))
        dot.add_edge(pydot.Edge(attack3,def3))
        dot.add_edge(pydot.Edge(attack4,def1))
        ...
        dot.add_edge(pydot.Edge(root,attack3))
        #3 lvl 
        attack4 = pydot.Node('Access from outside intended scope', shape='box')
        dot.add_node(attack4)
        dot.add_edge(pydot.Edge(attack,attack4))

        attack5 = pydot.Node('Vulnerable container image', shape='box')
        dot.add_node(attack5)
        defense1 = pydot.Node('Automated Secure Application Deployment')
        dot.add_node(defense1)
        dot.add_edge(pydot.Edge(attack5,defense1))
        dot.add_edge(pydot.Edge(attack2,attack5))
        attack6 = pydot.Node('Container running as root', shape='box')
        defense2 = pydot.Node('Least Privilege')
        dot.add_edge(pydot.Edge(attack6,defense2))
        dot.add_node(attack6)
        dot.add_edge(pydot.Edge(attack2,attack6))
        attack7 = pydot.Node('Vulnerable Runtime', shape='box')
        defese3 = pydot.Node('Automated Secure Application Deployment')
        dot.add_node(attack7)
        dot.add_edge(pydot.Edge(attack7,defese3))
        dot.add_node(attack7)
        dot.add_edge(pydot.Edge(attack3,attack7))
        attack8 = pydot.Node('Overprivileged role for resource', shape='box')
        defense4 = pydot.Node('Least Privilege')
        dot.add_node(attack8)
        dot.add_edge(pydot.Edge(attack3,attack8))
        dot.add_node(defense4)
        dot.add_edge(pydot.Edge(attack8,defense4))
        defense0 = pydot.Node('Data Protection by Design and Default')
        dot.add_node(defense0)
        dot.add_edge(pydot.Edge(attack4, defense0))
        defense99 = pydot.Node('OS Hardening and Base Controls')
        dot.add_node(defense99)
        dot.add_edge(pydot.Edge(attack7, defense99))
        dot.add_edge(pydot.Edge(attack5, defense99))
        ...
        dot.add_node(attack2)
        dot.add_edge(pydot.Edge(root,attack2))
        attack99 = pydot.Node(f'User makes data public incidentally', shape='box')
        dot.add_node(attack99)
        dot.add_edge(pydot.Edge(root,attack99))
        defense0 = pydot.Node('Data Protection by Design and Default')
        dot.add_node(defense0)
        dot.add_edge(pydot.Edge(attack99, defense0))
        #3 lvl
        ...
        dot.add_node(attack5)
        dot.add_edge(pydot.Edge(attack2,attack5))
        defense1 = pydot.Node('Application Vulnerability Remediation')
        dot.add_node(defense1)
        dot.add_edge(pydot.Edge(attack5,defense1))

        defense2 = pydot.Node(f'User access review')
        dot.add_node(attack2)
        dot.add_edge(pydot.Edge(attack3,defense2))
        dot.add_edge(pydot.Edge(attack4,defense2))
        dot.write_png(f'artifacts/atd/{threat.replace("/","_")}_{resource_name}_atd.png')
    else:
        return
